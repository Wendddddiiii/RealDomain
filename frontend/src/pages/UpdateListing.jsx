...
export default function CreateListing() {
    ...
    console.log('pets there', formData);
    ...
    return (
        <main className="p-3 max-4-xl mx-auto">
            <h1 className='text-3xl font-semibold text-center my-7 text-yellow-800'>Update a Listing</h1>
            <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-5'>
                <div className="flex flex-col gap-5 flex-1">
                    <input onChange={handleChange} value={formData.name} type="text" placeholder='Name' className='border p-3 rounded-lg' id='name' maxLength='50' minLength='10' required></input>
                    ...
                    <div className="flex gap-5 flex-wrap">
                        ...
                        <div className="flex gap-2">
                            <input type="checkbox" id ='petsAllowed' className="w-5" onChange={handleChange} checked={formData.petsAllowed}></input>
                            <span>Pets Allowed</span>
                        </div>
                    </div>   
                    <div className="flex flex-wrap gap-6">
                        <div className="flex items-center gap-2">
                            <input type="number" id="bedrooms" min='1' max='12' required className="p-3 border border-gray-300 rounded-lg" onChange={handleChange} value={formData.bedrooms}></input>
                            <p>Beds</p>
                        </div>
                        <div className="flex items-center gap-2">
                            <input type="number" id="bathrooms" min='1' max='8' required className="p-3 border border-gray-300 rounded-lg" onChange={handleChange} value={formData.bathrooms}></input>
                            <p>Baths</p>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        {formData.offer && (
                            <div className="flex items-center gap-2">
                                <input type="number" id="discountPrice" min='0' max='1000000' required className="p-3 border border-gray-300 rounded-lg" onChange={handleChange} value={formData.discountPrice}></input>
                                <div className="flex flex-col items-left">
                ...
                <div className="flex flex-col flex-1 gap-4">
                    <p className="font-semibold text-yellow-800">Upload photos of your listing here
                        <span className="font-normal text-gray-800 ml-2">(max 6)</span>                    
                    </p>
                    ...
                    <button disabled={loading || uploading} className="p-3 bg-yellow-400 text-white rounded-lg hover:opacity-80 disabled:opacity-60">{loading ? 'Updating...': 'Update Listing'}</button>
