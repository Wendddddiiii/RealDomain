  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form className='flex flex-col gap-4'onSubmit={handleSubmit}>
        ...
        <button disabled={loading} className="bg-amber-300 text-white p-3 rounded-lg uppercase hover:opacity-70 disabled:opacity-60">{loading ? 'Loading...': 'Sign Up'}</button>
        <OAuth />
      </form>
      <div className='flex gap-2 mt-5'>
        <p>Have an account?</p>
        <Link to={"/sign-in"}>
          <span className='text-yellow-800'>Sign in</span>
        </Link>
      </div>
